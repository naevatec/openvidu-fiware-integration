...
OpenVidu Fiware Integration is a project that integrates OpenVidu with Fiware through a web API to simplify the access to the OpenVidu video capabilities for Fiware applications.

This project is distributed in three modules:

1. **[fiware-integration](fiware-integration/README.md)**: the core of the project. It is a web API that simplifies the connection to OpenVidu and integrates it with Fiware.
2. **[openvidu-filters-client](openvidu-filters-client/README.md)**: an OpenVidu client that allows the core to handle the filters and events of the cameras.
3. **[fiware-integration-manager](fiware-integration-manager/README.md)** (*the demo*): a web application that allows to connect to an Orion instance, retrieve the available cameras, visualize their streams, modify the filters that are associated to them and catch the events they generate.

![Fiware Integration Schema](docs/fiware-integration-schema.png)
